...
import { Alert, AlertDescription } from "@/components/ui/alert";
import { 
  Plus, Search, Filter, FileText, Calendar, Eye, Edit, 
  CheckCircle, XCircle, Package, ArrowRight, AlertTriangle
} from "lucide-react";
...
interface Requisition {
  ...
  requester: string;
  department: string;
  status: "draft" | "pending_dept" | "pending_senior" | "approved" | "rejected" | "PO Created";
  inventoryAvailable: boolean;
  inventoryQuantity: number;
  budgetStatus: "within" | "exceeds" | "pending";
  approvalType: "inventory" | "purchase" | "pending";
...
const [selectedRequisition, setSelectedRequisition] = useState<Requisition | null>(null);
  ...
  const getFilteredData = () => {
    let filtered = requisitionsData.filter(
      (item) =>
        item.requestNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.item.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const columns = [
    ...
    { 
      ...
      header: "Est. Cost",
      render: (item: Requisition) => (
        <span>₨{item.totalEstimatedCost.toLocaleString()}</span>
      ),
    },
    {
      key: "requester",
    ...
    },
{
  ...
  return (
    <AppLayout
      ...
      actions={
        <>
          {/* Use the new CreateRequisitionDialog component */}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {requisitionsData.filter(r => r.status === "pending_dept" || r.status === "pending_senior").length}
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {requisitionsData.filter(r => r.status === "approved").length}
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {requisitionsData.filter(r => r.status === "PO Created").length}
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  ₨{requisitionsData.reduce((sum, r) => sum + r.totalEstimatedCost, 0).toLocaleString()}
                </p>
              </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <Tabs defaultValue="all" className="w-full sm:w-auto" onValueChange={setActiveTab}>
              <TabsList>
                ...
                <TabsTrigger value="approved">Approved</TabsTrigger>
                <TabsTrigger value="PO Created">PO Created</TabsTrigger>
